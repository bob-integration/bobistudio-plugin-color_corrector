...
def split_yuv(yuv_bytes):
    """yuv420p bytes → (Y, U, V) numpy uint8 arrays."""
    y = np.frombuffer(yuv_bytes[:Y_SIZE], dtype=np.uint8).reshape(HEIGHT, WIDTH).copy()
    u = np.frombuffer(yuv_bytes[Y_SIZE:Y_SIZE+UV_SIZE], dtype=np.uint8).reshape(HEIGHT//2, WIDTH//2).copy()
    v = np.frombuffer(yuv_bytes[Y_SIZE+UV_SIZE:],       dtype=np.uint8).reshape(HEIGHT//2, WIDTH//2).copy()
    return y, u, v
...
def yuv_to_rgb(y, u, v):
    ...
    uf = u.repeat(2, axis=0).repeat(2, axis=1).astype(np.float32)
    vf = v.repeat(2, axis=0).repeat(2, axis=1).astype(np.float32)
...
def rgb_to_yuv(rgb):
    ...
    u_u8 = np.clip(u[::2,::2], 0, 255).astype(np.uint8)
    v_u8 = np.clip(v[::2,::2], 0, 255).astype(np.uint8)
    return y_u8, u_u8, v_u8

def appliquer_correction(yuv_bytes, p):
    """Applique les params de correction. Renvoie yuv420p bytes."""
